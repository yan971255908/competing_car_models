...
import { BookOpen, Download, FileSpreadsheet, Sparkles, Upload } from 'lucide-react';
import { Button, GlassCard } from '@/components/ui';
import { competitors } from '@/lib/api';
import { CompetitorReviewPanel } from '@/components/competitors/CompetitorReviewPanel';
import type { CompetitorAIExtractResult, CompetitorAIStatus } from '@/types/competitorAI';
...
function TextSourceModal({ vehicles, technologies, initialSourceId, onClose, onEvidenceCreated, onGoToReview }: any) {
  ...
  const [aiStatus, setAIStatus] = useState<CompetitorAIStatus | null>(null);
  const [aiResult, setAIResult] = useState<CompetitorAIExtractResult | null>(null);
  const [isExtracting, setIsExtracting] = useState(false);
  const [extractionSourceId, setExtractionSourceId] = useState<string | null>(null);
  const [extractionSourceTitle, setExtractionSourceTitle] = useState('');
  const detailIdRef = useRef<string | null>(null);
  const clearAIResult = () => {
    setAIResult(null);
    setExtractionSourceTitle('');
  };
  ...
  const loadSources = async (preferredId?: string) => {
    ...
    if (targetId) {
      if (detailIdRef.current && detailIdRef.current !== targetId) clearAIResult();
      const nextDetail = await competitors.sourceDetail(targetId);
      setDetail(nextDetail);
      detailIdRef.current = nextDetail.id;
    } else {
      setDetail(null);
  ...
  useEffect(() => {
    loadSources(initialSourceId)
      .catch((err: any) => setError(err?.message || '来源文档加载失败'))
      .finally(() => setIsLoading(false));
    competitors.competitorAIStatus()
      .then((status) => { setAIStatus(status); setAIStatusError(''); })
      .catch(() => { setAIStatus(null); setAIStatusError('AI服务状态获取失败'); });
  }, []);
  const openSource = async (id: string) => {
    ...
    setIsEditing(false);
    try {
      const nextDetail = await competitors.sourceDetail(id);
      setDetail(nextDetail);
      detailIdRef.current = nextDetail.id;
    }
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setDetail(updated);
      detailIdRef.current = updated.id;
      clearAIResult();
      setIsEditing(false);
  ...
  const deleteSource = async () => {
    if (!detail || isExtracting || !window.confirm(`确认删除来源文档“${detail.title}”吗？`)) return;
    setIsSaving(true); setError('');
    try {
  ...
  const extractCandidates = async () => {
    if (!detail || !aiStatus?.configured) return;
    if (!window.confirm('AI 只会生成待审核候选，不会直接写入正式竞品库。是否继续？')) return;
    const sourceId = detail.id;
    const sourceTitle = detail.title || '未命名来源文档';
    setExtractionSourceId(sourceId);
    setExtractionSourceTitle(sourceTitle);
    setIsExtracting(true); setError(''); setAIResult(null);
    try {
      const result = await competitors.extractSourceCandidates({ source_document_id: sourceId, max_candidates: 20 });
      if (result.source_document_id !== sourceId) throw new Error('AI 提取结果与来源文档不一致');
      if (detailIdRef.current === sourceId) setAIResult(result);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'AI 候选提取失败');
    } finally {
  ...
  return <div className="fixed inset-0 z-[105] bg-black/65 backdrop-blur-sm flex items-center justify-center p-4">
    <GlassCard className="w-full max-w-[1180px] max-h-[92vh] border-white/10 shadow-2xl overflow-hidden flex flex-col">
      <div className="px-5 py-4 border-b border-white/10 flex items-center gap-3"><BookOpen size={18} className="text-cyan-300" /><div><div className="text-sm font-black text-white">文本资料导入</div><div className="text-[10px] text-white/35 mt-0.5">来源文档 · 正文查看 · 手工证据绑定</div></div><button onClick={onClose} disabled={isExtracting} title={isExtracting ? 'AI 提取期间不能关闭窗口' : '关闭'} className="ml-auto p-2 rounded hover:bg-white/10 text-white/45 disabled:opacity-30 disabled:cursor-not-allowed"><X size={18} /></button></div>
      <div className="p-5 overflow-y-auto custom-scrollbar space-y-4">
        {error && <div className="text-xs text-rose-300 bg-rose-500/10 border border-rose-500/20 rounded px-3 py-2">{error}</div>}
        {aiStatusError && <div className="text-xs text-amber-300 bg-amber-500/10 border border-amber-500/20 rounded px-3 py-2">{aiStatusError}</div>}
        <div className="border border-white/10 rounded overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex flex-col md:flex-row gap-2 md:items-center"><label className={`flex-1 border border-dashed border-white/15 bg-white/[0.03] rounded px-3 py-2 ${isExtracting ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}><input type="file" accept=".txt,text/plain" className="hidden" disabled={isExtracting} onChange={(event) => setTextFile(event.target.files?.[0] || null)} /><span className="text-[11px] text-white/55">{textFile?.name || '选择 .txt 文件'}</span></label><Button variant="ghost" size="sm" onClick={uploadText} disabled={!textFile || isSaving || isExtracting}><Upload size={14} />上传 TXT</Button><Button size="sm" onClick={saveTextSource} disabled={isSaving || isExtracting}>{isSaving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}保存来源文档</Button></div>
          </div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-4 min-h-[480px]">
          <div className="border border-white/10 rounded overflow-hidden flex flex-col">
            ...
            <div className="p-3 border-b border-white/10 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2"><select aria-label="来源类型筛选" value={sourceTypeFilter} onChange={(event) => setSourceTypeFilter(event.target.value)} className="competitor-select bg-white/5 border border-white/10 rounded px-2 py-1.5 text-[10px] text-white"><option value="">全部来源类型</option>{sourceTypeOptions.map((type) => <option key={type} value={type}>{sourceTypeLabel[type] || type}</option>)}</select><select aria-label="证据状态筛选" value={hasEvidenceFilter} onChange={(event) => setHasEvidenceFilter(event.target.value)} className="competitor-select bg-white/5 border border-white/10 rounded px-2 py-1.5 text-[10px] text-white"><option value="">全部证据状态</option><option value="true">已有证据</option><option value="false">暂无证据</option></select></div>
              <Button variant="ghost" size="sm" className="w-full" onClick={applySourceFilters} disabled={isExtracting}><Filter size={13} />应用筛选</Button>
            </div>
            <div className="flex-1 max-h-[620px] overflow-y-auto custom-scrollbar divide-y divide-white/5">{isLoading ? <div className="p-8 flex justify-center"><Loader2 className="animate-spin text-cyan-300" /></div> : sources.length === 0 ? <div className="p-8 text-center text-[11px] text-white/30">暂无来源文档</div> : sources.map((source) => <button key={source.id} disabled={isExtracting} onClick={() => openSource(source.id)} className={`w-full text-left px-4 py-3 hover:bg-white/[0.04] disabled:cursor-not-allowed ${isExtracting && detail?.id !== source.id ? 'opacity-35' : ''} ${detail?.id === source.id ? 'bg-cyan-500/[0.08]' : ''}`}><div className="text-xs font-bold text-white/80 line-clamp-2">{source.title}</div><div className="mt-2 flex flex-wrap gap-1"><span className="px-1.5 py-0.5 rounded bg-cyan-500/10 text-cyan-200 text-[9px]">{sourceTypeLabel[source.source_type] || source.source_type}</span>{source.file_name && <span className="px-1.5 py-0.5 rounded bg-white/5 text-white/40 text-[9px]">{source.file_name}</span>}</div>{source.source_url && <div className="mt-2 text-[9px] text-cyan-200/45 truncate">{source.source_url}</div>}<div className="mt-2 flex justify-between text-[9px] text-white/30"><span>{source.raw_text_length} 字</span><span>{source.evidence_count} 条证据</span></div><div className="mt-1 text-[9px] text-white/20">{source.created_at ? new Date(source.created_at).toLocaleString() : ''}</div></button>)}</div>
          </div>
          <div className="border border-white/10 rounded overflow-hidden">{!detail ? <div className="h-full flex items-center justify-center text-xs text-white/30">选择来源文档查看详情</div> : <div className="h-full flex flex-col"><div className="px-4 py-3 bg-white/[0.03] border-b border-white/10 flex items-start gap-3"><div className="min-w-0"><div className="text-sm font-bold text-white/85 truncate">{detail.title}</div><div className="mt-1 text-[10px] text-white/35 truncate">{sourceTypeLabel[detail.source_type] || detail.source_type}{detail.source_url ? ` · ${detail.source_url}` : ''}</div></div><div className="ml-auto flex items-center gap-1"><Button size="sm" onClick={extractCandidates} disabled={!aiStatus?.configured || isExtracting || isEditing} title={!aiStatus?.configured ? 'AI 服务未配置' : `使用 ${aiStatus.model}`} >{isExtracting && extractionSourceId === detail.id ? <Loader2 size={13} className="animate-spin" /> : <Sparkles size={13} />}{aiStatus?.configured ? (isExtracting && extractionSourceId === detail.id ? '提取中' : 'AI 提取候选') : 'AI 服务未配置'}</Button><button title="编辑来源文档" disabled={isExtracting} onClick={beginEdit} className="p-2 rounded hover:bg-white/10 text-white/40 hover:text-cyan-200 disabled:opacity-30 disabled:cursor-not-allowed"><Edit3 size={14} /></button><button title="删除来源文档" disabled={isExtracting} onClick={deleteSource} className="p-2 rounded hover:bg-rose-500/10 text-white/40 hover:text-rose-300 disabled:opacity-30 disabled:cursor-not-allowed"><X size={14} /></button></div></div><div className="p-4 space-y-4 overflow-y-auto custom-scrollbar">
            {aiResult && aiResult.source_document_id === detail.id && <div className="border border-violet-500/25 bg-violet-500/[0.06] rounded p-3 space-y-3"><div className="flex items-center gap-2 text-xs font-bold text-violet-100"><Sparkles size={14} />AI 提取完成</div><div className="text-[10px] text-white/45">来源文档：{extractionSourceTitle}</div><div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-[10px]"><div>提取 <b className="text-white">{aiResult.extracted_count}</b></div><div>创建 <b className="text-emerald-300">{aiResult.created_count}</b></div><div>重复 <b className="text-white">{aiResult.skipped_duplicates}</b></div><div>非原文 <b className="text-amber-300">{aiResult.skipped_ungrounded}</b></div><div>无效 <b className="text-rose-300">{aiResult.skipped_invalid}</b></div></div>{aiResult.warnings.length > 0 && <div className="space-y-1 text-[10px] text-amber-200">{aiResult.warnings.map((warning, index) => <div key={index}>{warning}</div>)}</div>}<Button size="sm" onClick={onGoToReview} disabled={aiResult.created_count === 0}>前往审核中心</Button></div>}
...
export const CompetitorLibrary: React.FC = () => {
  ...
  const [reviewOrigin, setReviewOrigin] = useState<'' | 'ai'>('');
  ...
  return <div className="competitor-library w-full relative">
    ...
    {textSourceOpen && <TextSourceModal vehicles={vehicles} technologies={technologies} initialSourceId={textSourceTarget} onClose={() => setTextSourceOpen(false)} onEvidenceCreated={loadData} onGoToReview={() => { setTextSourceOpen(false); setReviewOrigin('ai'); setWorkspace('review'); }} />}
    </>}
  </div>;
};
